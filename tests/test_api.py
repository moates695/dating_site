...
def test_page_url_redirects_to_trailing_slash(client):
    response = client.get(f"/d/{TEST_TOKEN}", follow_redirects=False)
    assert response.status_code == 308
    assert response.headers["location"] == f"/d/{TEST_TOKEN}/"


def test_serves_the_index(client):
    response = client.get(f"/d/{TEST_TOKEN}/")
...
def test_serves_an_asset(client):
    response = client.get(f"/d/{TEST_TOKEN}/style.css")
    assert response.status_code == 200
    assert "color: red" in response.text


def test_security_headers_are_set(client):
    headers = client.get(f"/d/{TEST_TOKEN}/").headers
...
def test_unknown_token_is_not_found(client):
    assert client.get(f"/d/{UNKNOWN_TOKEN}/").status_code == 404


def test_malformed_token_is_not_found(client):
    assert client.get("/d/NOT-A-TOKEN/").status_code == 404


def test_traversal_is_rejected(client):
    assert client.get(f"/d/{TEST_TOKEN}/../secret.txt").status_code == 404
    assert client.get(f"/d/{TEST_TOKEN}/%2e%2e/secret.txt").status_code == 404


def test_context_before_any_response(client):
    body = client.get(f"/api/d/{TEST_TOKEN}/context").json()
    assert body == {"display_name": "Test Person One", "submitted": False, "submitted_at": None}


def test_context_for_unknown_token(client):
    assert client.get(f"/api/d/{UNKNOWN_TOKEN}/context").status_code == 404


def test_submit_stores_the_response(client, fake_db):
    response = client.post(f"/api/d/{TEST_TOKEN}/submit", json=PAYLOAD)
    assert response.status_code == 200
...
def test_submit_accepts_an_arbitrary_answer_shape(client, fake_db):
    ...
    client.post(f"/api/d/{TEST_TOKEN}/submit", json={"summary": "x", "answers": answers})
...
def test_submit_rejects_a_bad_payload(client, fake_db):
    response = client.post(f"/api/d/{TEST_TOKEN}/submit", json={"answers": "nope"})
    assert response.status_code == 400
    assert fake_db.responses == []


def test_submit_for_unknown_token(client, fake_db):
    assert client.post(f"/api/d/{UNKNOWN_TOKEN}/submit", json=PAYLOAD).status_code == 404
    assert fake_db.responses == []


def test_submit_is_rate_limited(client):
    for _ in range(10):
        assert client.post(f"/api/d/{TEST_TOKEN}/submit", json=PAYLOAD).status_code == 200
    assert client.post(f"/api/d/{TEST_TOKEN}/submit", json=PAYLOAD).status_code == 429
