...
def test_the_address_is_hashed_not_stored(client, fake_db):
    client.get(f"/api/d/{TEST_TOKEN}/context", headers={"cf-connecting-ip": "203.0.113.9"})
...
def test_the_user_agent_is_recorded_and_capped(client, fake_db):
    client.get(f"/api/d/{TEST_TOKEN}/context", headers={"user-agent": "x" * 500})
...
def test_a_failed_write_does_not_break_the_page(client, fake_db):
    ...
    assert client.get(f"/d/{TEST_TOKEN}/").status_code == 200
    assert client.get(f"/api/d/{TEST_TOKEN}/context").status_code == 200


def test_an_unknown_token_records_nothing(client, fake_db):
    client.get(f"/d/{UNKNOWN_TOKEN}/")
    client.get(f"/api/d/{UNKNOWN_TOKEN}/context")
    assert fake_db.views == []
...
def test_a_failed_send_leaves_the_view_unmarked(settings, fake_db, monkeypatch):
    ...
    with client:
        response = client.get(f"/api/d/{TEST_TOKEN}/context")
